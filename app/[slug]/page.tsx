...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!(slug in PLACEHOLDER_PAGES)) {
    return buildPlaceholderPageMetadata(slug);
  }
...
export default async function ComingSoonPage({ params }: PageProps) {
  ...
  const displayTitle = getPlaceholderPageTitle(slug);
        ...
        <div className="relative flex flex-col gap-4">
          ...
          <div className="grid grid-cols-12 gap-3">
            <div className="col-span-5 space-y-3">
              <div className="h-14 rounded-xl border border-white/10 bg-white/10 shadow-inner shadow-black/20 backdrop-blur" />
              <div className="h-14 rounded-xl border border-white/10 bg-white/10 shadow-inner shadow-black/20 backdrop-blur" />
              <div className="h-14 rounded-xl border border-white/10 bg-white/10 shadow-inner shadow-black/20 backdrop-blur" />
            </div>
            <div className="col-span-7 space-y-3">
              <div className="flex gap-3">
                <div className="h-14 flex-1 rounded-xl border border-white/10 bg-gradient-to-r from-brand-500/40 to-indigo-400/30 shadow-lg shadow-brand-500/10" />
                <div className="h-14 w-16 rounded-xl border border-white/10 bg-white/10" />
              </div>
              <div className="flex gap-3">
                <div className="h-14 w-20 rounded-xl border border-white/10 bg-white/10" />
                <div className="h-14 flex-1 rounded-xl border border-white/10 bg-gradient-to-r from-emerald-400/30 to-cyan-400/30 shadow-lg shadow-emerald-400/10" />
              </div>
              <div className="flex gap-3">
                <div className="h-14 flex-1 rounded-xl border border-white/10 bg-white/10" />
                <div className="h-14 w-24 rounded-xl border border-white/10 bg-white/10" />
                <div className="h-14 w-16 rounded-xl border border-white/10 bg-white/10" />
              </div>
            </div>
          </div>
